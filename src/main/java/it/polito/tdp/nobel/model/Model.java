...
public class Model {
	...
	private void cerca2(Set<Esame> parziale, int L, int m) { //COMPLESSITA' 2^N
		...
		if(L==esami.size()) //questa è la fine di un ramo
			return;
	...
	private void cerca1(Set<Esame> parziale, int L, int m) {
		...
		for(Esame e:esami) {
			if(!parziale.contains(e)) {
				...
				parziale.remove(e);	//se ci fosse stata una lista il backtracking si fa con lista.remove(lista.size()-1)		}
		}
